    ...
    if func_key == "total_distance":
        ...
        def control_f(simulation_time:float, veh_obj:SimulationVehicle, veh_plan:VehiclePlan, rq_dict:Dict[Any,PlanRequest], routing_engine:NetworkBase)->float:
            ...
            for rid, boarding_info_list in veh_plan.pax_info.items():
                ...
                # Safety check for incomplete boarding_info_list
                if not boarding_info_list or len(boarding_info_list) < 2:
                    LOG.warning(f"Incomplete boarding_info_list for rid {rid}: {boarding_info_list} - using pickup time as dropoff time")
                    drop_off_time = boarding_info_list[0] if boarding_info_list else rq_time + 300  # fallback: 5 minutes
                else:
                    drop_off_time = boarding_info_list[1]
